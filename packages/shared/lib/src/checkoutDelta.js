 ...
 */
export function computeCheckoutDelta(now, expectedCheckoutAt) {
    const nowMs = now.getTime();
    const expMs = expectedCheckoutAt.getTime();
    const status = nowMs <= expMs ? 'remaining' : 'late';
...
export function formatCheckoutDelta(delta) {
...
//# sourceMappingURL=checkoutDelta.js.map