...
export const roomStatus = pgEnum("room_status", ['DIRTY', 'CLEANING', 'CLEAN', 'OCCUPIED'])
...
export const clubEvents = pgTable("club_events", {
	...
	index("idx_club_events_amount").using("btree", table.amountCents.asc().nullsLast().op("int4_ops"), table.occurredAt.desc().nullsFirst().op("int4_ops")).where(sql`(amount_cents IS NOT NULL)`),
