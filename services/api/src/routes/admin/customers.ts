    ...
    async (request, reply) => {
      ...
      try {
        ...
        const blocksResult = await db.execute<AgreementBlockRow>(
          sql`SELECT cb.id, cb.visit_id, cb.block_type::text as block_type, cb.starts_at, cb.ends_at, cb.rental_type::text as rental_type, r.number as resource_number, r.kind as resource_kind, cb.agreement_signed, cb.agreement_signed_at, (sig.signature_png_base64 IS NOT NULL) as has_pdf, visit_orders.payment_total, visit_orders.payment_method, sig.signature_png_base64, sig.signature_strokes_json, sig.created_at as signature_created_at, sig.agreement_version, sig.agreement_text_snapshot FROM checkin_blocks cb LEFT JOIN inventory_resources r ON r.id = cb.resource_id LEFT JOIN LATERAL (SELECT SUM(o.total::numeric) as payment_total, (ARRAY_AGG(o.payment_method ORDER BY o.total::numeric DESC))[1] as payment_method FROM orders o WHERE o.visit_id = cb.visit_id AND o.status = 'PAID') visit_orders ON TRUE LEFT JOIN LATERAL (SELECT signature_png_base64, signature_strokes_json, created_at, agreement_version, agreement_text_snapshot FROM agreement_signatures WHERE checkin_block_id = cb.id ORDER BY created_at DESC LIMIT 1) sig ON TRUE WHERE cb.visit_id IN (${sql.join(visitIds.map(id => sql`${id}::uuid`), sql`, `)}) ORDER BY cb.starts_at DESC, cb.id DESC`
        );
