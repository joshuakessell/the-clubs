...
export function registerRoomManagementRoutes(fastify: FastifyInstance): void {
  ...
  fastify.post('/v1/admin/room-management/rooms', { schema: { body: CreateRoomSchema }, preHandler: [requireAuth, requireAdmin, idempotencyKey] }, async (request, reply) => {
  ...
  fastify.patch<{ Params: { roomId: string } }>('/v1/admin/room-management/rooms/:roomId', { schema: { body: UpdateRoomSchema }, preHandler: [requireAuth, requireAdmin] }, async (request, reply) => {
  ...
  fastify.post<{ Params: { roomId: string } }>('/v1/admin/room-management/rooms/:roomId/set-status', { schema: { body: SetStatusBodySchema }, preHandler: [requireAuth, requireAdmin, idempotencyKey] }, async (request, reply) => {
  ...
  fastify.post('/v1/admin/room-management/lockers', { schema: { body: CreateLockerSchema }, preHandler: [requireAuth, requireAdmin, idempotencyKey] }, async (request, reply) => {
  ...
  fastify.post<{ Params: { lockerId: string } }>('/v1/admin/room-management/lockers/:lockerId/set-status', { schema: { body: SetStatusBodySchema }, preHandler: [requireAuth, requireAdmin, idempotencyKey] }, async (request, reply) => {
    const body = request.body as z.infer<typeof SetStatusBodySchema>;
