...
export function registerRoomManagementRoutes(fastify: FastifyInstance): void {
  ...
  fastify.patch<{ Params: { resourceId: string } }>('/v1/admin/room-management/rooms/:resourceId', { preHandler: [requireAuth, requireAdmin] }, async (request, reply) => {
    ...
    try { return reply.send(await updateRoom(request.params.resourceId, body.type, body.floor)); }
  ...
  fastify.post<{ Params: { resourceId: string } }>('/v1/admin/room-management/rooms/:resourceId/set-status', { preHandler: [requireAuth, requireAdmin, idempotencyKey] }, async (request, reply) => {
    const body = request.body as z.infer<typeof SetStatusBodySchema>;
    try { return reply.send(await setRoomStatus(request.params.resourceId, body.status)); }
  ...
  fastify.post<{ Params: { resourceId: string } }>('/v1/admin/room-management/lockers/:resourceId/set-status', { preHandler: [requireAuth, requireAdmin, idempotencyKey] }, async (request, reply) => {
    const body = request.body as z.infer<typeof SetStatusBodySchema>;
    try { return reply.send(await setLockerStatus(request.params.resourceId, body.status)); }
