...
import { listActiveStaff, loginWithPin, changeStaffPin, logoutSession, reauthWithPin, getReauthWebauthnOptions, verifyReauthWebauthn } from '../services/authService';
...
export async function authRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/v1/auth/login-pin', {
    ...
    try {
      ...
      const verifyHash = hashSessionToken(result.sessionToken);
      request.log.info(
        {
  ...
  const ChangePinSchema = z.object({
    currentPin: z.string().regex(/^\d{6}$/, 'PIN must be exactly 6 digits').optional(),
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      try {
        const reauthOkUntil = await reauthWithPin(request.staff.staffId, body.pin, tokenHash);
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      try {
        const reauthOkUntil = await verifyReauthWebauthn(
          request.staff.staffId,
          deviceId,
          tokenHash,
          origin,
          request.body.credentialResponse
        );
