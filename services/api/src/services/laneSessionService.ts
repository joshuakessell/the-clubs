...
export async function startLaneSession(
  ...
  return db.transaction(async (tx) => {
    ...
    if (requestedCustomerId) {
      ...
      if (!customerId) {
        const newCustomer = await tx.execute<{ id: string }>(
          sql`INSERT INTO customers (name, created_at, updated_at) VALUES (${idScanValue || 'Customer'}, NOW(), NOW()) RETURNING id`
        );
    ...
    const existingSession = await tx.execute<Record<string, unknown>>(
      sql`SELECT id, status FROM lane_sessions WHERE lane_id = ${laneId} AND status IN ('IDLE', 'ACTIVE', 'AWAITING_CUSTOMER') ORDER BY created_at DESC LIMIT 1`
    );
    ...
    if (existingSession.rows.length > 0 && (existingSession.rows[0] as unknown as LaneSessionRow).status !== 'COMPLETED') {
      ...
      const updateResult = await tx.execute<Record<string, unknown>>(
        sql`UPDATE lane_sessions SET customer_display_name = ${customerName}, membership_number = ${membershipNumber}, customer_id = ${customerId}, status = 'ACTIVE',
    ...
    if (session.customer_id) {
      const customerInfo = await tx.execute<Record<string, unknown>>(
        sql`SELECT past_due_balance, membership_card_type, membership_valid_until FROM customers WHERE id = ${session.customer_id}`
      );
