...
import { LANE_SESSION_COLS, ORDER_COLS } from "../../checkin/types";
...
import { autoAssignResource } from "../agreementService";
import type { FlowActor, FlowStep, FlowCommandType } from "../../checkin/schemas";

/**
 * Structured error for flow command failures.
 * Extends Error so stack traces, Sentry, and Fastify error hooks work correctly.
 */
export class FlowCommandError extends Error {
...
async function handleCreateOrderForPayment(client: Parameters<typeof getLaneFeatureFlags>[0], params: { session: LaneSessionRow; sessionId: string }) {
  ...
  let rentalType = (session.desired_rental_type ?? session.proposed_rental_type ?? 'LOCKER') as import('../../pricing/engine').RentalType;
  if (session.waitlist_desired_type && session.backup_rental_type) {
    rentalType = session.backup_rental_type as import('../../pricing/engine').RentalType;
  }
  ...
  const sessionWaitlistType = session.waitlist_desired_type as import('../../pricing/engine').RentalType | undefined;
...
async function acquirePaymentReservationLock(
  ...
  const assigned = await autoAssignResource(tx, rentalType);
