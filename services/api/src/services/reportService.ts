...
export async function getCashTotals() {
  const totals = await db.execute<{ total: string | null }>(sql`SELECT COALESCE(SUM(amount), 0)::numeric(10,2) as total FROM payment_intents WHERE status = 'PAID' AND paid_at >= date_trunc('day', NOW()) AND paid_at < date_trunc('day', NOW()) + INTERVAL '1 day'`);
  const byMethod = await db.execute<{ payment_method: string | null; total: string | null }>(sql`SELECT payment_method, COALESCE(SUM(amount), 0)::numeric(10,2) as total FROM payment_intents WHERE status = 'PAID' AND paid_at >= date_trunc('day', NOW()) AND paid_at < date_trunc('day', NOW()) + INTERVAL '1 day' GROUP BY payment_method`);
  const byRegister = await db.execute<{ register_number: number | null; total: string | null }>(sql`SELECT register_number, COALESCE(SUM(amount), 0)::numeric(10,2) as total FROM payment_intents WHERE status = 'PAID' AND paid_at >= date_trunc('day', NOW()) AND paid_at < date_trunc('day', NOW()) + INTERVAL '1 day' GROUP BY register_number ORDER BY register_number NULLS LAST`);
...
export async function getDailySummary(targetDate: string) {
  const revenue = await db.execute<{ total: string }>(sql`SELECT COALESCE(SUM(amount), 0)::numeric(10,2) AS total FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${targetDate}::date AND paid_at < ${targetDate}::date + INTERVAL '1 day'`);
  const revenueByMethod = await db.execute<{ payment_method: string | null; total: string }>(sql`SELECT payment_method, COALESCE(SUM(amount), 0)::numeric(10,2) AS total FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${targetDate}::date AND paid_at < ${targetDate}::date + INTERVAL '1 day' GROUP BY payment_method`);
  ...
  const tips = await db.execute<{ total: string }>(sql`SELECT COALESCE(SUM(tip), 0)::numeric(10,2) AS total FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${targetDate}::date AND paid_at < ${targetDate}::date + INTERVAL '1 day' AND tip > 0`);
...
export async function getRevenueTrend(days: number) {
  ...
  const result = await db.execute<{ day: string; total: string; transaction_count: number }>(sql`SELECT TO_CHAR(paid_at::date, 'YYYY-MM-DD') AS day, COALESCE(SUM(amount), 0)::numeric(10,2) AS total, COUNT(*)::int AS transaction_count FROM payment_intents WHERE status = 'PAID' AND paid_at >= NOW() - ${clampedDays}::int * INTERVAL '1 day' GROUP BY paid_at::date ORDER BY day`);
...
export async function getStaffProductivity(from: string, to: string, staffId?: string) {
  ...
  const revenueResult = await db.execute<{ staff_id: string; total: string; tx_count: number }>(sql`SELECT s.id AS staff_id, COALESCE(SUM(pi.amount), 0)::numeric(10,2) AS total, COUNT(pi.id)::int AS tx_count FROM staff s LEFT JOIN payment_intents pi ON pi.paid_by_staff_id = s.id AND pi.status = 'PAID' AND pi.paid_at >= ${from}::date AND pi.paid_at < ${to}::date + INTERVAL '1 day' WHERE s.active = true ${staffFilterClause} GROUP BY s.id`);
...
export async function getOperationsSummary(from: string, to: string) {
  const revenue = await db.execute<{ total: string; count: number; avg_tx: string }>(sql`SELECT COALESCE(SUM(amount), 0)::numeric(10,2) AS total, COUNT(*)::int AS count, COALESCE(AVG(amount), 0)::numeric(10,2) AS avg_tx FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${from}::date AND paid_at < ${to}::date + INTERVAL '1 day'`);
  const tips = await db.execute<{ total: string }>(sql`SELECT COALESCE(SUM(tip), 0) AS total FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${from}::date AND paid_at < ${to}::date + INTERVAL '1 day' AND tip > 0`);
  ...
  const occupancy = await db.execute<{ total: number; occupied: number }>(sql`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE status = 'OCCUPIED')::int AS occupied FROM rooms`);
...
export async function getRevenueBreakdown(from: string, to: string) {
  const byMethod = await db.execute<{ payment_method: string | null; total: string; count: number }>(sql`SELECT COALESCE(payment_method, 'UNKNOWN') AS payment_method, COALESCE(SUM(amount), 0)::numeric(10,2) AS total, COUNT(*)::int AS count FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${from}::date AND paid_at < ${to}::date + INTERVAL '1 day' GROUP BY payment_method`);
  const byDow = await db.execute<{ dow: number; day_name: string; total: string; count: number }>(sql`SELECT EXTRACT(DOW FROM paid_at)::int AS dow, TO_CHAR(paid_at, 'Dy') AS day_name, COALESCE(SUM(amount), 0)::numeric(10,2) AS total, COUNT(*)::int AS count FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${from}::date AND paid_at < ${to}::date + INTERVAL '1 day' GROUP BY dow, day_name ORDER BY dow`);
  const byRentalType = await db.execute<{ rental_type: string; total: string; count: number }>(sql`SELECT cb.rental_type::text AS rental_type, COALESCE(SUM(pi.amount), 0)::numeric(10,2) AS total, COUNT(*)::int AS count FROM payment_intents pi JOIN lane_sessions ls ON ls.payment_intent_id = pi.id JOIN checkin_blocks cb ON cb.session_id = ls.id WHERE pi.status = 'PAID' AND pi.paid_at >= ${from}::date AND pi.paid_at < ${to}::date + INTERVAL '1 day' GROUP BY cb.rental_type`);
...
export async function getLaborCost(from: string, to: string, hourlyRate: number) {
  ...
  const revenueByStaff = await db.execute<{ staff_id: string; total: string }>(sql`SELECT paid_by_staff_id AS staff_id, COALESCE(SUM(amount), 0)::numeric(10,2) AS total FROM payment_intents WHERE status = 'PAID' AND paid_at >= ${from}::date AND paid_at < ${to}::date + INTERVAL '1 day' AND paid_by_staff_id IS NOT NULL GROUP BY paid_by_staff_id`);
