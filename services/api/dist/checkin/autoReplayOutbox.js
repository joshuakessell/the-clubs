...
exports.startAutoReplayOutbox = startAutoReplayOutbox;
 ...
 */
function startAutoReplayOutbox(params) {
    const { pool, cloudApiBase, kioskToken, signal } = params;
    const log = params.log ?? ((...args) => console.log('[auto-replay]', ...args));
    ...
    const getHealthInterval = () => {
        ...
        return Math.min(HEALTH_CHECK_MAX_MS, HEALTH_CHECK_BASE_MS * Math.pow(2, consecutiveHealthFailures - 1));
    };
    ...
    const replayBatch = async () => {
        ...
        let replayed = 0;
        try {
            const pending = await client.query(`SELECT id, lane_id, session_id, command_id, actor, type, payload_json, replay_attempts
         FROM offline_command_outbox
         WHERE replayed_at IS NULL
           AND replay_attempts < $2
         ORDER BY created_at ASC
         LIMIT $1`, [REPLAY_BATCH_SIZE, MAX_REPLAY_ATTEMPTS]);
            for (const row of pending.rows) {
                ...
                try {
                    ...
                    await client.query(`UPDATE offline_command_outbox
             SET replayed_at = NOW(),
