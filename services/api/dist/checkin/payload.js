...
function parseFlowStep(step) {
    ...
    return step && validSteps.has(step) ? step : undefined;
}
function parseAgreementMethod(method) {
    return method === 'MANUAL' || method === 'DIGITAL' ? method : undefined;
}
function formatDob(dob) {
    if (!dob)
        return undefined;
    return (0, utils_1.toDate)(dob)?.toISOString().slice(0, 10) ?? String(dob).slice(0, 10);
}
function formatTimestamp(val) {
    if (!val)
        return undefined;
    return (0, utils_1.toDate)(val)?.toISOString() ?? String(val);
}
function formatMembershipValidUntil(raw) {
...
async function fetchCustomerForSession(session) {
...
async function buildRenewalLedger(session, checkinVisitId) {
    const ledgerItems = [];
    let total = 0;
    if (!checkinVisitId)
...
function buildCheckinMembership(session, customer) {
...
function buildCheckinRental(session, customer) {
    ...
    if (!rentalType || (!session.selection_confirmed && !isWaitlisted))
        return [];
    const items = [];
    const rentalLabel = { LOCKER: 'Locker', STANDARD: 'Standard Room', DOUBLE: 'Double Room', SPECIAL: 'Special Room', GYM_LOCKER: 'Gym Locker' };
...
async function buildCheckinLedger(session, customer, pastDueBalance, paymentLineItems, checkinVisitId) {
    const ledgerItems = [];
    if (pastDueBalance > 0) {
    ...
    if (paymentLineItems) {
        ...
        ledgerItems.push(...buildCheckinMembership(session, customer), ...buildCheckinRental(session, customer));
    }
    if (checkinVisitId) {
        const charges = await db_1.db.execute((0, drizzle_orm_1.sql) `
      SELECT oli.kind as type, oli.total as amount
...
async function fetchCustomerVisitHistory(customerId) {
    let customerLastVisitAt;
    let activeVisitId;
    let activeBlockEndsAt;
    if (customerId) {
        const lastVisitResult = await db_1.db.execute((0, drizzle_orm_1.sql) `
    ...
    `);
        ...
        const activeVisitResult = await db_1.db.execute((0, drizzle_orm_1.sql) `
...
async function fetchBlockForSession(sessionId) {
...
async function fetchPaymentIntent(sessionId, orderId) {
    if (orderId) {
        const intentResult = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT ${drizzle_orm_1.sql.raw(types_1.ORDER_COLS)} FROM orders WHERE id = ${orderId} LIMIT 1`);
        return intentResult.rows[0];
    }
    const intentResult = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT ${drizzle_orm_1.sql.raw(types_1.ORDER_COLS)} FROM orders WHERE lane_session_id = ${sessionId} ORDER BY created_at DESC LIMIT 1`);
    return intentResult.rows[0];
}
