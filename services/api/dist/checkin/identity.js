...
exports.computeSha256Hex = computeSha256Hex;
exports.computeIdScanIdentityHash = computeIdScanIdentityHash;
exports.normalizeScanText = normalizeScanText;
exports.isLikelyAamvaPdf417Text = isLikelyAamvaPdf417Text;
exports.extractAamvaIdentity = extractAamvaIdentity;
exports.normalizePersonNameForMatch = normalizePersonNameForMatch;
exports.splitNamePartsForMatch = splitNamePartsForMatch;
exports.scoreNameMatch = scoreNameMatch;
exports.passesFuzzyThresholds = passesFuzzyThresholds;
exports.calculateAge = calculateAge;
exports.getIdScanIssue = getIdScanIssue;
exports.getIdScanIssueMessage = getIdScanIssueMessage;
exports.parseMembershipNumber = parseMembershipNumber;
const crypto_1 = __importDefault(require("crypto"));
const aamva_parser_1 = require("aamva-parser");
...
function computeIdScanIdentityHash(params) {
...
function isLikelyAamvaPdf417Text(raw) {
...
function extractAamvaFieldMap(raw) {
...
function inferAamvaIdType(fieldMap) {
    ...
    const hasLicenseSignal = licenseSignals.some((value) => !['NONE', 'N/A', 'NA', 'ID', 'IDENTIFICATION'].includes(value));
    return hasLicenseSignal ? 'DRIVERS_LICENSE' : 'STATE_ID';
}
function parseAamvaDateToISO(value) {
    if (!value)
        return undefined;
...
function isCleanParsedAamvaValue(value) {
    ...
    if (/(?:^|[^A-Z])D[A-Z]{2}(?:[^A-Z]|$)/.test(s))
        return false;
    if (/\b(DAQ|DBB|DBD|DCS|DAC|DAA|DAJ|DCI)\b/.test(s))
        return false;
    return true;
}
function extractAamvaIdentity(rawNormalized) {
    ...
    try {
        const parsed = (0, aamva_parser_1.Parse)(rawNormalized);
        // Only trust parsed values if they look clean AND we are missing that field from fieldMap.
        const parsedDob = parsed?.dateOfBirth instanceof Date
        ...
        if (!out.issuer && isCleanParsedAamvaValue(parsed?.state))
            out.issuer = parsed.state.trim();
        if (!out.dob && parsedDob)
            out.dob = parsedDob;
...
function passesFuzzyThresholds(score) {
    return (score.score >= FUZZY_MIN_OVERALL &&
...
function getIdScanIssue(params) {
...
function getIdScanIssueMessage(issue) {
    switch (issue) {
        case 'ID_EXPIRED':
 ...
 */
function parseMembershipNumber(scanValue) {
