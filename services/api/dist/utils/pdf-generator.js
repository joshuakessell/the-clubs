...
function extractBase64FromDataUrlOrRaw(input) {
...
function parseAgreementHtmlToBlocks(inputHtml) {
    ...
    const blocks = [];
    let current = null;
    const styleStack = [{ bold: false, italic: false }];
    ...
    for (const token of tokens) {
        ...
        if (!isEnd) {
            ...
            if (tag === 'ul' || tag === 'ol') {
                listStack.push({ kind: tag, index: 0 });
                continue;
            }
            if (tag === 'li') {
                const top = listStack[listStack.length - 1];
...
function formatZonedDateTime(dt, timeZone) {
...
async function tryEmbedClubLogoPng(pdfDoc) {
    ...
    const candidates = [
        envPath,
        node_path_1.default.resolve(process.cwd(), '../../apps/customer-kiosk/src/assets/the-clubs-logo.png'),
        node_path_1.default.resolve(process.cwd(), '../apps/customer-kiosk/src/assets/the-clubs-logo.png'),
        node_path_1.default.resolve(process.cwd(), 'apps/customer-kiosk/src/assets/the-clubs-logo.png'),
    ].filter(Boolean);
...
async function generateAgreementPdf(params) {
    ...
    const timesBoldItalic = await pdfDoc.embedFont(pdf_lib_1.StandardFonts.TimesRomanBoldItalic);
    ...
    const measure = (text, style, fontSize) => {
        const f = pickBodyFont(style);
        return f.widthOfTextAtSize(text, fontSize);
    };
    const splitLongToken = (token, fontSize, maxWidth) => {
        ...
        for (const ch of token.text) {
            ...
            if (f.widthOfTextAtSize(candidate, fontSize) <= maxWidth) {
                ...
                if (chunk)
                    out.push({ text: chunk, style: token.style });
                chunk = ch;
            }
        }
        if (chunk)
    ...
    const layoutRunsToLines = (runs, fontSize, maxWidth) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (line.length === 0 && w > maxWidth) {
                ...
                for (let pi = 0; pi < parts.length; pi++) {
                    ...
                    if (width + pw > maxWidth && line.length > 0)
                        pushLine();
                    line.push(p);
                    width += pw;
                    if (width >= maxWidth)
    ...
    const drawFooter = (page, pageIndex, totalPages) => {
    ...
    const drawTextLineRuns = (page, x, y, line, fontSize) => {
        let cursorX = x;
    ...
    currentPage.drawRectangle({
        ...
        borderColor: (0, pdf_lib_1.rgb)(0.6, 0.6, 0.6),
        borderWidth: 1,
    });
