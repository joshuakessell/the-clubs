 ...
 */
const OfferUpgradeSchema = zod_1.z.object({
    roomId: zod_1.z.string().uuid(),
});
/**
 * Schema for completing upgrade.
 */
const CompleteUpgradeSchema = zod_1.z.object({
    waitlistId: zod_1.z.string().uuid(),
    paymentIntentId: zod_1.z.string().uuid(),
});
/**
 * Schema for cancelling waitlist entry.
 */
const CancelWaitlistSchema = zod_1.z.object({
    waitlistId: zod_1.z.string().uuid(),
...
async function waitlistRoutes(fastify) {
    ...
    fastify.get('/v1/waitlist', {
        ...
        try {
            await (0, expireWaitlist_1.expireWaitlistEntries)(fastify);
        }
        catch (error) {
    ...
    fastify.post('/v1/waitlist/:id/offer', {
        ...
        try {
            const result = await (0, db_1.serializableTransaction)(async (client) => {
                ...
                const waitlistResult = await client.query(`SELECT w.*, v.ended_at as visit_ended_at, cb.ends_at as block_ends_at
             FROM waitlist w
             ...
             FOR UPDATE`, [id]);
                ...
                const roomResult = await client.query(`SELECT id, number, type, status, assigned_to_customer_id FROM rooms WHERE id = $1 FOR UPDATE`, [body.roomId]);
                ...
                const reservationConflict = await client.query(`SELECT id
             FROM inventory_reservations
             WHERE resource_type = 'room'
               AND resource_id = $1
               AND released_at IS NULL
               AND (waitlist_id IS NULL OR waitlist_id <> $2)
             LIMIT 1`, [body.roomId, id]);
                ...
                const reserved = await client.query(`SELECT w.id
             FROM waitlist w
             JOIN visits v ON v.id = w.visit_id
             JOIN checkin_blocks cb ON cb.id = w.checkin_block_id
             ...
             LIMIT 1`, [body.roomId, id]);
                ...
                const desiredExpiryRes = await client.query(`SELECT offer_expires_at FROM waitlist WHERE id = $1 FOR UPDATE`, [id]);
                const existingExpiresAt = desiredExpiryRes.rows[0]?.offer_expires_at ?? null;
                const tenFromNow = new Date(Date.now() + 10 * 60 * 1000);
                const nextExpiresAt = existingExpiresAt && existingExpiresAt.getTime() > tenFromNow.getTime()
                    ? existingExpiresAt
                    : tenFromNow;
             ...
             ON CONFLICT DO NOTHING`, [body.roomId, id, nextExpiresAt]);
                await client.query(`UPDATE inventory_reservations
             SET expires_at = $1
             WHERE released_at IS NULL
               AND kind = 'UPGRADE_HOLD'
               AND waitlist_id = $2`, [nextExpiresAt, id]);
           ...
           SET status = 'OFFERED',
               ...
               updated_at = NOW()
           WHERE id = $3`, [body.roomId, nextExpiresAt, id]);
                // Log audit
    ...
    fastify.post('/v1/waitlist/:id/cancel', {
        ...
        try {
            const result = await (0, db_1.transaction)(async (client) => {
                // Get waitlist entry
                const waitlistResult = await client.query(`SELECT * FROM waitlist WHERE id = $1 FOR UPDATE`, [id]);
                if (waitlistResult.rows.length === 0) {
                    throw { statusCode: 404, message: 'Waitlist entry not found' };
                }
                const waitlist = waitlistResult.rows[0];
