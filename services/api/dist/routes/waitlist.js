...
async function waitlistRoutes(fastify) {
    ...
    fastify.post('/v1/waitlist/:id/offer', { schema: { body: OfferUpgradeSchema }, preHandler: [middleware_1.requireAuth] }, async (request, reply) => {
        if (!request.staff)
            return reply.status(401).send({ error: 'Unauthorized' });
    ...
    fastify.post('/v1/waitlist/:id/cancel', { schema: { body: CancelWaitlistSchema }, preHandler: [middleware_1.requireAuth] }, async (request, reply) => {
        if (!request.staff)
            return reply.status(401).send({ error: 'Unauthorized' });
        const body = request.body;
