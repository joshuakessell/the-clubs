...
function registerCheckinLaneSessionRoutes(fastify) {
    ...
    fastify.post('/v1/checkin/lane/:laneId/start', {
        ...
        const { customerId: requestedCustomerId, idScanValue, membershipScanValue, visitId, renewalHours, } = body;
        try {
            const result = await (0, db_1.transaction)(async (client) => {
                ...
                let membershipNumber = membershipScanValue
                    ? (0, identity_1.parseMembershipNumber)(membershipScanValue)
                    : null;
             ...
             LIMIT 1`, [requestedCustomerId]);
                    ...
                    idScanIssue = (0, identity_1.getIdScanIssue)({
                        dob: customer.dob,
               ...
               LIMIT 1`, [membershipNumber]);
                    ...
                    if (!customerId) {
                        const newCustomer = await client.query(`INSERT INTO customers (name, created_at, updated_at)
               VALUES ($1, NOW(), NOW())
               RETURNING id`, [idScanValue || 'Customer']);
                ...
                const resolveVisitBlocks = async (activeVisitId) => {
                    const blocksResult = await client.query(`SELECT starts_at, ends_at FROM checkin_blocks 
             WHERE visit_id = $1 ORDER BY ends_at DESC`, [activeVisitId]);
           ...
           LIMIT 1`, [laneId]);
             ...
             RETURNING *`, [
                ...
                if (session.customer_id) {
                    const customerInfo = await client.query(`SELECT past_due_balance FROM customers WHERE id = $1`, [session.customer_id]);
            ...
            if (result.idScanIssue) {
                return reply.status(403).send({
                    error: (0, identity_1.getIdScanIssueMessage)(result.idScanIssue),
