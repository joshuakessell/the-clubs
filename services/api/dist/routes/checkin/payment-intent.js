...
function registerCheckinPaymentIntentRoutes(fastify) {
    ...
    fastify.post('/v1/checkin/lane/:laneId/create-payment-intent', {
        ...
        try {
           ...
           LIMIT 1`, [laneId]);
                ...
                const dueIntents = await client.query(`SELECT * FROM payment_intents
           ...
           ORDER BY created_at DESC`, [session.id]);
             ...
             SET amount = $1,
                 quote_json = $2,
                 updated_at = NOW()
             WHERE id = $3`, [quote.total, JSON.stringify(quote), intent.id]);
                }
                else {
                    const intentResult = await client.query(`INSERT INTO payment_intents 
             (lane_session_id, amount, status, quote_json)
             VALUES ($1, $2, 'DUE', $3)
             RETURNING *`, [session.id, quote.total, JSON.stringify(quote)]);
           ...
           SET payment_intent_id = $1,
               ...
               updated_at = NOW()
           WHERE id = $3`, [intent.id, JSON.stringify(quote), session.id]);
               ...
               VALUES ($1, $2, $3, $4, $5)
               ON CONFLICT (session_id, command_id) DO NOTHING`, [session.id, commandId, 'EMPLOYEE', 'SET_STEP', { step: 'PAYMENT' }]);
                    await client.query(`UPDATE lane_sessions
               ...
               WHERE id = $2`, [commandId, session.id]);
                ...
                return {
                    ...
                    amount: (0, utils_1.toNumber)(intent.amount),
                    quote,
                };
            });
    ...
    fastify.post('/v1/payments/:id/mark-paid', {
        ...
        try {
            const result = await (0, db_1.transaction)(async (client) => {
                // Get payment intent
                const intentResult = await client.query(`SELECT * FROM payment_intents WHERE id = $1`, [id]);
                ...
                if (intent.status === 'PAID') {
                 ...
                 WHERE id = $2
                   AND paid_by_staff_id IS NULL`, [staffId, intent.id]);
                    }
                    const quote = parsePaymentIntentQuote(intent.quote_json);
                    if (squareTransactionId || intent.square_transaction_id) {
                        await client.query(`INSERT INTO external_provider_refs (provider, entity_type, internal_id, external_id)
                 VALUES ('square', 'payment', $1, $2)
                 ON CONFLICT DO NOTHING`, [intent.id, squareTransactionId || intent.square_transaction_id]);
                    }
                    await ensureAuditTrail(intent, quote);
                    return { paymentIntentId: intent.id, status: 'PAID', alreadyPaid: true };
                }
                // Mark as paid
                const updatedIntent = await client.query(`UPDATE payment_intents
           SET status = 'PAID',
               paid_at = NOW(),
           ...
           RETURNING *`, [
                ...
                if (squareTransactionId || paidIntent.square_transaction_id) {
                    await client.query(`INSERT INTO external_provider_refs (provider, entity_type, internal_id, external_id)
               VALUES ('square', 'payment', $1, $2)
               ON CONFLICT DO NOTHING`, [paidIntent.id, squareTransactionId || paidIntent.square_transaction_id]);
                }
                ...
                // Handle regular check-in payment
                    ...
                    if (sessionResult.rows.length > 0) {
                        ...
                        await client.query(`UPDATE lane_sessions SET status = $1, updated_at = NOW() WHERE id = $2`, [newStatus, session.id]);
                        return {
                ...
                return {
                    paymentIntentId: paidIntent.id,
                    status: 'PAID',
                    laneSessionToBroadcast: null,
                };
            });
            if (result.laneSessionToBroadcast) {
                const { payload } = await (0, db_1.transaction)((client) => (0, payload_1.buildFullSessionUpdatedPayload)(client, result.laneSessionToBroadcast.sessionId));
