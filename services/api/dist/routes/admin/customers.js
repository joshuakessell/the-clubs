...
function registerAdminCustomerRoutes(fastify) {
    ...
    fastify.patch('/v1/admin/customers/:id', {
        ...
        const UpdateSchema = zod_1.z
            .object({
            pastDueBalance: zod_1.z.number().min(0).optional(),
        })
        ...
        try {
            const result = await (0, db_1.transaction)(async (client) => {
                const existing = await client.query(`SELECT id, name, membership_number, primary_language, past_due_balance
           FROM customers
           WHERE id = $1
           FOR UPDATE`, [request.params.id]);
                ...
                const updated = await client.query(`UPDATE customers
           SET ${updates.join(', ')}, updated_at = NOW()
           WHERE id = $${idx}
           RETURNING id, name, membership_number, primary_language, past_due_balance`, params);
            ...
            return reply.send({
                ...
                primaryLanguage: result.primary_language || null,
     ...
     */
    fastify.get('/v1/admin/customers/:customerId/agreements', {
        preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin],
    }, async (request, reply) => {
        const { customerId } = request.params;
        const limit = Math.min(Math.max(parseInt(request.query.limit || '25', 10) || 25, 1), 100);
        try {
            const visitsResult = await (0, db_1.query)(`SELECT id, started_at, ended_at
           FROM visits
           WHERE customer_id = $1
           ...
           LIMIT $2`, [customerId, limit]);
            ...
            const blocksResult = await (0, db_1.query)(`
          SELECT
            cb.id,
            cb.visit_id,
