...
exports.registerAdminCustomerRoutes = registerAdminCustomerRoutes;
...
function registerAdminCustomerRoutes(fastify) {
    fastify.get('/v1/admin/customers', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        ...
        try {
            ...
            return reply.send({ customers: result.rows.map((r) => ({ id: r.id, name: r.name, dob: r.dob, membershipNumber: r.membership_number, membershipCardType: r.membership_card_type, membershipValidUntil: r.membership_valid_until?.toISOString() ?? null, primaryLanguage: r.primary_language || null, pastDueBalance: parseFloat(String(r.past_due_balance || 0)), lastVisit: r.last_visit?.toISOString() ?? null })) });
        }
        catch (e) {
    ...
    fastify.patch('/v1/admin/customers/:id', { preHandler: [middleware_1.requireReauthForAdmin] }, async (request, reply) => {
        ...
        try {
            ...
            return reply.send({ id: result.id, name: result.name, membershipNumber: result.membership_number, primaryLanguage: result.primary_language || null, pastDueBalance: parseFloat(String(result.past_due_balance || 0)) });
        }
        catch (error) {
            if (error?.statusCode)
                return reply.status(error.statusCode).send({ error: error.message ?? 'Failed to update customer' });
            request.log.error(error, 'Failed to update customer');
            return reply.status(500).send({ error: 'Internal server error' });
        }
    });
    fastify.get('/v1/admin/customers/:customerId/agreements', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
