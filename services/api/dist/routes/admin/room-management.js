...
function registerRoomManagementRoutes(fastify) {
    ...
    fastify.patch('/v1/admin/room-management/rooms/:resourceId', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        const body = request.body;
        if (!body.type && body.floor === undefined)
            return reply.status(400).send({ error: 'At least one field (type, floor) is required' });
        try {
            return reply.send(await (0, roomManagementService_1.updateRoom)(request.params.resourceId, body.type, body.floor));
        }
        catch (e) {
    ...
    fastify.post('/v1/admin/room-management/rooms/:resourceId/set-status', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin, idempotency_1.idempotencyKey] }, async (request, reply) => {
        const body = request.body;
        try {
            return reply.send(await (0, roomManagementService_1.setRoomStatus)(request.params.resourceId, body.status));
        }
        catch (e) {
            const httpErr = (0, utils_1.getHttpError)(e);
            if (httpErr) {
    ...
    fastify.post('/v1/admin/room-management/lockers/:resourceId/set-status', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin, idempotency_1.idempotencyKey] }, async (request, reply) => {
        const body = request.body;
        try {
            return reply.send(await (0, roomManagementService_1.setLockerStatus)(request.params.resourceId, body.status));
        }
        catch (e) {
            const httpErr = (0, utils_1.getHttpError)(e);
            if (httpErr) {
