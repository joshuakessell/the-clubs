...
// ---------------------------------------------------------------------------
const AnalyticsRangeSchema = zod_1.z.object({
...
function registerAdminClubAnalyticsRoutes(fastify) {
    ...
    fastify.get('/v1/admin/club-analytics/employee-summary', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        let parsed;
        try {
            parsed = AnalyticsRangeSchema.parse(request.query);
        }
        catch (error) {
        ...
        try {
            ...
            const checkins = await (0, db_1.query)(`SELECT staff_id, staff_name, COUNT(*)::text as count
           FROM club_events
           ...
           WHERE event_type IN ('EMPLOYEE_CLOCK_IN', 'EMPLOYEE_CLOCK_OUT')
             AND occurred_at >= $1 AND occurred_at <= $2
             AND staff_id IS NOT NULL
           ORDER BY staff_id, occurred_at`, [from, to]);
    ...
    fastify.get('/v1/admin/club-analytics/daily-summary', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        ...
        const { from, to, tz } = parseRange(parsed);
        try {
            const result = await (0, db_1.query)(`SELECT to_char(date_trunc('day', occurred_at AT TIME ZONE $3), 'YYYY-MM-DD') as bucket,
