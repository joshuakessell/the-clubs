...
function registerAdminReportRoutes(fastify) {
    ...
    fastify.get('/v1/admin/reports/cash-totals', {
        ...
        try {
            const totals = await (0, db_1.query)(`SELECT COALESCE(SUM(amount), 0)::numeric(10,2) as total
         FROM payment_intents
         WHERE status = 'PAID'
           AND paid_at >= date_trunc('day', NOW())
           AND paid_at <  date_trunc('day', NOW()) + INTERVAL '1 day'`);
            const byMethod = await (0, db_1.query)(`SELECT payment_method, COALESCE(SUM(amount), 0)::numeric(10,2) as total
         FROM payment_intents
         WHERE status = 'PAID'
           AND paid_at >= date_trunc('day', NOW())
           AND paid_at <  date_trunc('day', NOW()) + INTERVAL '1 day'
         GROUP BY payment_method`);
            const byRegister = await (0, db_1.query)(`SELECT register_number, COALESCE(SUM(amount), 0)::numeric(10,2) as total
         FROM payment_intents
         WHERE status = 'PAID'
           AND paid_at >= date_trunc('day', NOW())
           AND paid_at <  date_trunc('day', NOW()) + INTERVAL '1 day'
    ...
    fastify.get('/v1/admin/reports/daily-summary', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            // Revenue
            const revenue = await (0, db_1.query)(`SELECT
            COALESCE(SUM(amount), 0)::numeric(10,2) AS total
          FROM payment_intents
          WHERE status = 'PAID'
            AND paid_at >= $1::date
            AND paid_at < $1::date + INTERVAL '1 day'`, [targetDate]);
            const revenueByMethod = await (0, db_1.query)(`SELECT payment_method, COALESCE(SUM(amount), 0)::numeric(10,2) AS total
          FROM payment_intents
          WHERE status = 'PAID'
            AND paid_at >= $1::date
            AND paid_at < $1::date + INTERVAL '1 day'
          GROUP BY payment_method`, [targetDate]);
            // Check-ins
          ...
          WHERE action_type = 'CHECK_IN'
            ...
            // Unique customers
            const uniqueCustomers = await (0, db_1.query)(`SELECT COUNT(DISTINCT customer_id)::int AS count
          FROM customer_activity_events
    ...
    fastify.get('/v1/admin/reports/staff-productivity', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            // Check-ins per staff
            const checkIns = await (0, db_1.query)(`SELECT
            s.id AS staff_id,
          ...
          ORDER BY s.name`, params);
            // Revenue per staff
            const revenueResult = await (0, db_1.query)(`SELECT
            s.id AS staff_id,
          ...
          GROUP BY s.id`, params);
            const revenueMap = new Map(revenueResult.rows.map((r) => [
                r.staff_id,
    ...
    fastify.get('/v1/admin/reports/staff-productivity-hourly', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            const hourlyCheckIns = await (0, db_1.query)(`SELECT
          ...
          GROUP BY hour
          ORDER BY hour`, [staffId, targetDate]);
            const hourlyRevenue = await (0, db_1.query)(`SELECT
    ...
    fastify.get('/v1/admin/reports/operations-summary', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
          ...
          WHERE action_category = 'CHECKIN'
            ...
            AND occurred_at < $2::date + INTERVAL '1 day'`, [from, to]);
            const checkOuts = await (0, db_1.query)(`SELECT COUNT(*)::int AS count
          FROM customer_activity_events
          ...
          WHERE occurred_at >= $1::date
            AND occurred_at < $2::date + INTERVAL '1 day'`, [from, to]);
            // Labor hours
            const labor = await (0, db_1.query)(`SELECT
          ...
          WHERE clock_in_at >= $1::date
            AND clock_in_at < $2::date + INTERVAL '1 day'`, [from, to]);
            // Occupancy (rooms currently occupied)
            const occupancy = await (0, db_1.query)(`SELECT
            COUNT(*)::int AS total,
            COUNT(*) FILTER (WHERE status = 'OCCUPIED')::int AS occupied
          FROM rooms`);
            // Overrides count
            const overrides = await (0, db_1.query)(`SELECT COUNT(*)::int AS count
          FROM audit_log
          WHERE action = 'OVERRIDE'
            AND created_at >= $1::date
            AND created_at < $2::date + INTERVAL '1 day'`, [from, to]);
            const dayCount = Math.max(1, Math.ceil((new Date(to + 'T23:59:59').getTime() - new Date(from + 'T00:00:00').getTime()) /
                (1000 * 60 * 60 * 24)));
            ...
            return reply.send({
                ...
                occupancy: {
                    ...
                    rate: (occupancy.rows[0]?.total ?? 0) > 0
                        ? Math.round(((occupancy.rows[0]?.occupied ?? 0) / (occupancy.rows[0]?.total ?? 1)) * 100)
                        : 0,
                },
    ...
    fastify.get('/v1/admin/reports/hourly-heatmap', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            // Activity heatmap
            const activity = await (0, db_1.query)(`SELECT
    ...
    fastify.get('/v1/admin/reports/revenue-breakdown', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            const to = request.query.to ?? today;
            // By payment method
            const byMethod = await (0, db_1.query)(`SELECT
          ...
          GROUP BY payment_method`, [from, to]);
            // By day of week
            const byDow = await (0, db_1.query)(`SELECT
            EXTRACT(DOW FROM paid_at)::int AS dow,
          ...
          ORDER BY dow`, [from, to]);
            // By rental type (via checkin_blocks)
            const byRentalType = await (0, db_1.query)(`SELECT
    ...
    fastify.get('/v1/admin/reports/labor-cost', { preHandler: [middleware_1.requireAuth, middleware_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            const scheduled = await (0, db_1.query)(`SELECT
            s.id AS employee_id,
          ...
          WHERE s.active = true
          GROUP BY s.id, s.name
          ORDER BY s.name`, [from, to]);
            // Actual hours per employee
            const actual = await (0, db_1.query)(`SELECT
            employee_id,
          ...
          GROUP BY employee_id`, [from, to]);
            ...
            const revenueByStaff = await (0, db_1.query)(`SELECT
          ...
          GROUP BY paid_by_staff_id`, [from, to]);
            ...
            return reply.send({
                ...
                totals: {
                    ...
                    revenuePerLaborHour: totalActual > 0 ? Math.round((totalRevenue / totalActual) * 100) / 100 : 0,
                },
                employees,
            });
        }
