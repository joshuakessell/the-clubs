        ...
        try {
            const result = await (0, db_1.serializableTransaction)(async (client) => {
                ...
                if (block.session_id) {
                    ...
                    if (laneSession?.payment_intent_id) {
                        const intentResult = await client.query(`SELECT id, amount, quote_json FROM payment_intents WHERE id = $1 LIMIT 1`, [
                            laneSession.payment_intent_id,
                        ]);
                        originalIntent = intentResult.rows[0];
                    }
                    else {
                        const intentResult = await client.query(`SELECT id, amount, quote_json
                 FROM payment_intents
                 WHERE lane_session_id = $1
                 ORDER BY created_at DESC
                 LIMIT 1`, [block.session_id]);
                        originalIntent = intentResult.rows[0];
                    }
                    originalLineItems =
           ...
           RETURNING id, amount`, [
                ...
                return {
                    waitlistId,
                    paymentIntentId: paymentIntent.id,
                    upgradeFee: typeof paymentIntent.amount === 'string'
                    ...
                    customerId: (await client.query(`SELECT customer_id FROM visits WHERE id = $1 LIMIT 1`, [waitlist.visit_id])).rows[0].customer_id,
                };
            });
    ...
    fastify.post('/v1/upgrades/complete', {
        ...
        try {
           ...
           SET assigned_to_customer_id = (SELECT customer_id FROM visits WHERE id = $1),
               ...
               updated_at = NOW()
           WHERE id = $2`, [waitlist.visit_id, newRoomId]);
           ...
           WHERE id = $3`, [newRoomId, waitlist.desired_tier, block.id]);
                // 8. Mark waitlist as COMPLETED
                await client.query(`UPDATE waitlist 
           SET status = 'COMPLETED', completed_at = NOW(), updated_at = NOW()
           WHERE id = $1`, [waitlistId]);
               ...
               VALUES ($1, $2, $3, $4, $5)`, [waitlist.visit_id, block.id, 'UPGRADE_FEE', upgradeAmount, paymentIntentId]);
                    }
                }
