...
async function orderRoutes(fastify) {
    fastify.post('/v1/orders', { schema: { body: CreateOrderSchema }, preHandler: [middleware_1.requireAuth, idempotency_1.idempotencyKey] }, async (request, reply) => {
        if (!request.staff)
            return reply.status(401).send({ error: 'Unauthorized' });
        const body = request.body;
        return reply.send(await (0, orderService_1.createOrder)(body, request.staff.staffId));
    });
    fastify.post('/v1/orders/:orderId/line-items', { schema: { body: AddLineItemsSchema }, preHandler: [middleware_1.requireAuth] }, async (request, reply) => {
        if (!request.staff)
            return reply.status(401).send({ error: 'Unauthorized' });
        const body = request.body;
