...
const CreateOrderSchema = zod_1.z.object({
    ...
    registerSessionId: zod_1.z.string().uuid().optional().nullable(),
    metadataJson: zod_1.z.record(zod_1.z.unknown()).optional().nullable(),
});
const LineItemSchema = zod_1.z.object({
    kind: zod_1.z.enum(['RETAIL', 'ADDON', 'UPGRADE', 'LATE_FEE', 'MANUAL']),
    ...
    fastify.post('/v1/orders/:orderId/receipt', { preHandler: [middleware_1.requireAuth] }, async (request, reply) => {
        ...
        try {
            const result = await (0, db_1.transaction)(async (client) => {
                ...
                const existingReceipt = await client.query(`SELECT id, receipt_number, issued_at, receipt_json
             FROM receipts
             WHERE order_id = $1
             LIMIT 1`, [order.id]);
