...
function registerVisitActiveRoutes(fastify) {
    fastify.get('/v1/visits/active', async (request, reply) => {
        try {
            ...
            let visitsResult;
            if (membershipNumber) {
                const raw = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT v.id, v.customer_id, v.started_at, v.ended_at, v.created_at, v.updated_at,
                  c.name as customer_name, c.membership_number
           FROM visits v
           JOIN customers c ON v.customer_id = c.id
           WHERE v.ended_at IS NULL AND c.membership_number = ${membershipNumber}
           ORDER BY v.started_at DESC`);
                visitsResult = { rows: raw.rows };
            }
           ...
           LIMIT 20`);
            ...
            const activeVisits = await Promise.all(visitsResult.rows.map(async (visit) => {
                const blocksRaw = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT id, visit_id, block_type, starts_at, ends_at, rental_type::text as rental_type, resource_id, session_id, agreement_signed, created_at, updated_at
             FROM checkin_blocks WHERE visit_id = ${visit.id} ORDER BY ends_at DESC`);
                const blocks = blocksRaw.rows;
