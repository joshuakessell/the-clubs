...
async function timeoffRoutes(fastify) {
     ...
     */
    fastify.get('/v1/schedule/time-off-requests', {
    ...
    fastify.post('/v1/schedule/time-off-requests', {
        ...
        try {
           ...
           VALUES ($1, $2, $3)
           RETURNING id`, [request.staff.staffId, body.day, body.reason ?? null]);
     ...
     */
    fastify.get('/v1/admin/time-off-requests', {
    ...
    fastify.patch('/v1/admin/time-off-requests/:requestId', {
        ...
        try {
            const updated = await (0, db_1.transaction)(async (client) => {
                ...
                await client.query(`UPDATE time_off_requests
           SET status = $1,
               decided_by = $2,
               decided_at = NOW(),
               decision_notes = $3,
               updated_at = NOW()
           WHERE id = $4`, [body.status, request.staff.staffId, body.decisionNotes ?? null, requestId]);
