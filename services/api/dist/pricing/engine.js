...
exports.calculateRenewalQuote = calculateRenewalQuote;
exports.getUpgradeFee = getUpgradeFee;
/**
 ...
 */
function isWeekdayDiscountWindow(date) {
 ...
 */
function hasValidSixMonthMembership(now, membershipCardType, membershipValidUntil) {
    ...
    const endOfDay = new Date(membershipValidUntil.getFullYear(), membershipValidUntil.getMonth(), membershipValidUntil.getDate(), 23, 59, 59, 999);
 ...
 */
function getBaseRoomPrice(rentalType, isWeekdayDiscount) {
    switch (rentalType) {
        case 'STANDARD':
 ...
 */
function getYouthRoomPrice(rentalType) {
    switch (rentalType) {
        case 'STANDARD':
            return 30;
        case 'DOUBLE':
 ...
 */
function getLockerPrice(rentalType, checkInTime, isYouth) {
 ...
 */
function getMembershipFee(checkInTime, customerAge, membershipCardType, membershipValidUntil) {
 ...
 */
function calculatePriceQuote(input) {
 ...
 */
function calculateRenewalQuote(input) {
 ...
 */
function getUpgradeFee(from, to) {
    const upgradeFees = {
        LOCKER: {
            STANDARD: 8,
            DOUBLE: 17,
