...
function rentalFromRoomType(type) {
    switch (type) {
        case shared_1.RoomType.STANDARD:
            return shared_1.RentalType.STANDARD;
        case shared_1.RoomType.DOUBLE:
            return shared_1.RentalType.DOUBLE;
        case shared_1.RoomType.SPECIAL:
            return shared_1.RentalType.SPECIAL;
        case shared_1.RoomType.LOCKER:
        default:
            return shared_1.RentalType.LOCKER;
    }
}
function pickAvailableRoom(rooms, preferred) {
    const cleanRooms = rooms.filter((r) => r.status === shared_1.RoomStatus.CLEAN ||
        r.status === shared_1.RoomStatus.DIRTY ||
        r.status === shared_1.RoomStatus.CLEANING);
    if (preferred) {
...
function pickLocker(lockers) {
    return lockers.length > 0 ? pick(lockers) : null;
}
function generateCustomers(count, now) {
    const customers = [];
    const names = new Set();
    for (let i = 0; i < count; i++) {
        ...
        const primaryLanguage = Math.random() < 0.15 ? 'ES' : 'EN';
        customers.push({
...
function planVisitsForCustomer(customerId, now, rooms, lockers, activeSlots, weekCounts, overdueMinutes = 0, hoursCheckedIn = 0) {
    const visits = [];
    ...
    for (let i = 0; i < visitCount; i++) {
        ...
        while (weekCounts.get(getWeekKey(adjustedStart)) &&
        ...
        if (rentalChoice < 0.15) {
            rental = shared_1.RentalType.GYM_LOCKER;
        }
        else if (rentalChoice < 0.45) {
            rental = shared_1.RentalType.LOCKER;
        }
        else if (rentalChoice < 0.75) {
            rental = shared_1.RentalType.STANDARD;
        }
        else if (rentalChoice < 0.92) {
            rental = shared_1.RentalType.DOUBLE;
        }
        else {
            rental = shared_1.RentalType.SPECIAL;
        }
        const blocks = [];
        let blockStart = adjustedStart;
        for (let b = 0; b < totalBlocks; b++) {
            const blockType = b === 0 ? shared_1.BlockType.INITIAL : shared_1.BlockType.RENEWAL;
            ...
            blocks.push({
                ...
                rental_type: rental,
                resource_id: null,
                has_tv_remote: rental !== shared_1.RentalType.LOCKER && rental !== shared_1.RentalType.GYM_LOCKER
                    ? Math.random() < 0.2
                    : false,
    ...
    if (activeSlots > 0) {
        ...
        const rentalChoices = [shared_1.RentalType.STANDARD, shared_1.RentalType.LOCKER, shared_1.RentalType.DOUBLE, shared_1.RentalType.STANDARD, shared_1.RentalType.LOCKER, shared_1.RentalType.SPECIAL];
        const rentalIdx = Math.floor(checkedInHours * 2) % rentalChoices.length;
        const rental = rentalChoices[rentalIdx] ?? shared_1.RentalType.STANDARD;
        const activeBlocks = [
            {
                id: (0, node_crypto_1.randomUUID)(),
                visit_id: '', // backfill
                block_type: shared_1.BlockType.INITIAL,
        ...
        const activeVisitId = (0, node_crypto_1.randomUUID)();
    ...
    for (const visit of visits) {
        for (const block of visit.blocks) {
            if (block.rental_type === shared_1.RentalType.LOCKER || block.rental_type === shared_1.RentalType.GYM_LOCKER) {
...
function generateDemoData(options) {
    ...
    for (const customer of customers) {
        ...
        const hoursCheckedIn = (!isOverdueSlot && activeRemaining > 0)
            ...
            : 0;
        const customerVisits = planVisitsForCustomer(customer.id, now, options.rooms, options.lockers, activeRemaining > 0 ? 1 : 0, weekCounts, overdueMinutes, hoursCheckedIn);
