    ...
    try {
       ...
       WHERE starts_at >= $1 AND starts_at <= $2`, [past14Days, next14Days]);
        ...
        for (let dayOffset = -14; dayOffset <= 14; dayOffset++) {
            ...
            const shiftAResult = await (0, index_1.query)(`INSERT INTO employee_shifts 
         (employee_id, starts_at, ends_at, shift_code, status, created_by)
         VALUES ($1, $2, $3, 'A', 'SCHEDULED', $4)
         RETURNING id`, [shiftAEmployee.id, shiftAStart, shiftAEnd, adminStaff.id]);
            ...
            const shiftBResult = await (0, index_1.query)(`INSERT INTO employee_shifts 
         (employee_id, starts_at, ends_at, shift_code, status, created_by)
         VALUES ($1, $2, $3, 'B', 'SCHEDULED', $4)
         RETURNING id`, [shiftBEmployee.id, shiftBStart, shiftBEnd, adminStaff.id]);
            ...
            const shiftCResult = await (0, index_1.query)(`INSERT INTO employee_shifts 
         (employee_id, starts_at, ends_at, shift_code, status, created_by)
         VALUES ($1, $2, $3, 'C', 'SCHEDULED', $4)
         RETURNING id`, [shiftCEmployee.id, shiftCStart, shiftCEnd, adminStaff.id]);
            ...
            if (dayOffset < 0) {
                ...
                if (scenarioA < 0.95) {
                    ...
                    const sessionAResult = await (0, index_1.query)(`INSERT INTO timeclock_sessions 
             (employee_id, shift_id, clock_in_at, clock_out_at, source)
             VALUES ($1, $2, $3, $4, 'OFFICE_DASHBOARD')
             RETURNING id`, [shiftAEmployee.id, shiftAId, clockIn, clockOut]);
             ...
             (employee_id, shift_id, clock_in_at, clock_out_at, source)
             VALUES ($1, $2, $3, $4, 'OFFICE_DASHBOARD')
             RETURNING id`, [shiftAEmployee.id, shiftAId, shiftAStart, shiftAEnd]);
                    timeclockSessionsCreated.push(sessionAResult.rows[0].id);
                }
                // Shift B timeclock
                ...
                if (scenarioB < 0.95) {
                    ...
                    const sessionBResult = await (0, index_1.query)(`INSERT INTO timeclock_sessions 
             (employee_id, shift_id, clock_in_at, clock_out_at, source)
             VALUES ($1, $2, $3, $4, 'OFFICE_DASHBOARD')
             RETURNING id`, [shiftBEmployee.id, shiftBId, clockIn, clockOut]);
                ...
                if (scenarioC < 0.95) {
                    ...
                    const sessionCResult = await (0, index_1.query)(`INSERT INTO timeclock_sessions 
             (employee_id, shift_id, clock_in_at, clock_out_at, source)
             VALUES ($1, $2, $3, $4, 'OFFICE_DASHBOARD')
             RETURNING id`, [shiftCEmployee.id, shiftCId, clockIn, clockOut]);
                ...
                for (const employee of todayStaff) {
                    ...
                    const existing = await (0, index_1.query)(`SELECT COUNT(*) as count
             FROM timeclock_sessions
             WHERE employee_id = $1 AND clock_out_at IS NULL`, [employee.id]);
                    if (parseInt(existing.rows[0]?.count || '0', 10) === 0) {
                        ...
                        const sessionResult = await (0, index_1.query)(`INSERT INTO timeclock_sessions 
               (employee_id, shift_id, clock_in_at, clock_out_at, source)
               VALUES ($1, $2, $3, NULL, 'OFFICE_DASHBOARD')
               RETURNING id`, [employee.id, shiftId, clockInTime]);
