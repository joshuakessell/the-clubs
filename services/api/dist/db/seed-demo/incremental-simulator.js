...
function isFridayOrSaturdayPeak(d) {
...
function visitStartRatePerHour(d) {
    ...
    if (isFridayOrSaturdayPeak(d))
        return 27; // capacity
    const hour = hourOf(d);
    let rate;
    if (hour >= 12 && hour <= 16)
        rate = 8; // afternoon lull
    else if (hour >= 17 && hour <= 19)
        rate = 18; // evening ramp-up
    else if (hour >= 20 && hour <= 23)
        rate = 24; // peak hours — busy
    else if (hour >= 0 && hour <= 3)
        rate = 27; // late night — at capacity
    else
        rate = 20; // early morning winding down
...
function sampleCheckoutDeltaMinutes(rng) {
    return pickWeighted(rng, [
        { item: 0, weight: 0.55 },
        { item: 5, weight: 0.18 },
        { item: 10, weight: 0.1 },
...
function generateNewCustomerData(rng, now) {
    const seq = ++newCustomerSeq;
    const firstName = FIRST_NAMES[Math.floor(rng() * FIRST_NAMES.length)];
    const lastName = LAST_NAMES[Math.floor(rng() * LAST_NAMES.length)];
    const name = `${firstName} ${lastName}`;
    const dob = new Date(1970 + Math.floor(rng() * 35), Math.floor(rng() * 12), 1 + Math.floor(rng() * 27));
...
async function appendIncrementalDemoSimulation(params) {
    ...
    const checkoutRequestEvents = [];
    // Late checkout events (capped at 2 per night)
    const lateCheckoutEvents = [];
    ...
    for (let i = 0; i < intervals && created < maxVisits; i += 1) {
        ...
        for (let j = 0; j < visitsInSlot && created < maxVisits; j += 1) {
            ...
            if (rng() < 0.8 && params.customers.length > 0) {
                ...
                await params.client.query(`INSERT INTO customers
         ...
         VALUES ($1, $2, $3, $4, NOW(), NOW())`, [visitId, start, end, customer.id]);
            ...
            await params.client.query(`INSERT INTO checkin_blocks
         (id, visit_id, block_type, starts_at, ends_at, locker_id, room_id,
          agreement_signed, agreement_signed_at, rental_type)
         VALUES ($1, $2, 'INITIAL', $3, $4, $5, $6, true, $7, $8)`, [checkinBlockId, visitId, start, scheduledEnd, lockerId, roomId, signedAt, rentalType]);
    ...
    if (untouchedCustomers.length > 0) {
        ...
        for (let gi = 0; gi < untouchedCustomers.length; gi++) {
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
            ...
            const checkinStartedAt = new Date(start.getTime() - 3 * 60 * 1000);
            await params.client.query(`INSERT INTO customer_activity_events
           (occurred_at, customer_id, action_type, action_category, source_app,
            actor_type, actor_staff_id, actor_staff_name, summary, metadata, search_blob, dedupe_key)
    ...
    for (const ev of checkoutEvents) {
      ...
      `, [
        ...
        if (rng() < 0.10) {
            ...
            const noteAt = new Date(ev.occurredAt.getTime() - Math.floor(rng() * 60) * 60 * 1000 // during the visit
            );
            await params.client.query(`INSERT INTO customer_notes
           (id, customer_id, created_at, created_by_staff_id, created_by_staff_name, source_app, note, is_important)
    ...
    for (const wl of waitlistEvents) {
       ...
       VALUES ($1, $2, $3, $4::rental_type, 'LOCKER'::rental_type, $5,
        ...
        await params.client.query(`UPDATE checkin_blocks SET waitlist_id = $1 WHERE id = $2`, [waitlistId, wl.checkinBlockId]);
        // Inventory reservation (released)
        await params.client.query(`INSERT INTO inventory_reservations
    ...
    for (const cr of checkoutRequestEvents) {
       ...
       VALUES ($1, $2, 'demo-kiosk-1', $3, 'VERIFIED',
        ...
        const matchingLate = lateCheckoutEvents.find((le) => le.checkinBlockId === cr.checkinBlockId && le.checkoutRequestId === null);
        if (matchingLate) {
