...
async function publishMetric(metricName, value, unit = 'Count', dimensions) {
    ...
    try {
        const dimensionEntries = dimensions
            ...
            : [];
        await cw.send(new client_cloudwatch_1.PutMetricDataCommand({
            Namespace: NAMESPACE,
            MetricData: [
                {
...
exports.BusinessMetrics = {
    /** Customer checked in */
    checkIn: (registerId) => publishMetric('CheckInCount', 1, 'Count', registerId ? { RegisterId: registerId } : undefined),
    /** Customer checked out */
    checkOut: () => publishMetric('CheckOutCount', 1, 'Count'),
    /** Payment completed */
    payment: (amountCents, method) => publishMetrics([
    ...
    override: (action) => publishMetric('OverrideCount', 1, 'Count', { Action: action }),
    /** Room occupancy snapshot */
    occupancy: (occupied, total) => publishMetric('OccupancyRate', total > 0 ? (occupied / total) * 100 : 0, 'Percent'),
    /** Staff clock-in */
    clockIn: () => publishMetric('StaffClockInCount', 1, 'Count'),
    /** API error occurred */
    apiError: (endpoint) => publishMetric('ApiErrorCount', 1, 'Count', { Endpoint: endpoint }),
};
