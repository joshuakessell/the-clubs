...
async function startLaneSession(input, staff) {
    ...
    return db_1.db.transaction(async (tx) => {
        ...
        if (requestedCustomerId) {
            const customerResult = await tx.execute((0, drizzle_orm_1.sql) `SELECT id, name, dob, id_expiration_date, membership_number, membership_card_type, membership_valid_until, banned_until, id_scan_hash
         FROM customers WHERE id = ${requestedCustomerId} LIMIT 1`);
           ...
           FROM customers WHERE membership_number = ${membershipNumber} LIMIT 1`);
            ...
            if (!customerId) {
                const newCustomer = await tx.execute((0, drizzle_orm_1.sql) `INSERT INTO customers (name, created_at, updated_at) VALUES (${idScanValue || 'Customer'}, NOW(), NOW()) RETURNING id`);
        ...
        const resolveVisitBlocks = async (activeVisitId) => {
            const blocksResult = await tx.execute((0, drizzle_orm_1.sql) `SELECT starts_at, ends_at FROM checkin_blocks WHERE visit_id = ${activeVisitId} ORDER BY ends_at DESC`);
            if (blocksResult.rows.length > 0) {
                blockEndsAtDate = (0, utils_1.toDate)(blocksResult.rows[0].ends_at) ?? null;
                for (const block of blocksResult.rows) {
                    const endsAt = (0, utils_1.toDate)(block.ends_at);
                    const startsAt = (0, utils_1.toDate)(block.starts_at);
                    if (endsAt && startsAt) {
        ...
        const resolveActiveAssignment = async (activeVisitId) => {
            const activeBlock = await tx.execute((0, drizzle_orm_1.sql) `SELECT cb.rental_type, cb.resource_id, r.number as resource_number, r.kind as resource_kind
        ...
        if (visitId) {
            const visitResult = await tx.execute((0, drizzle_orm_1.sql) `SELECT id, customer_id, started_at, ended_at FROM visits WHERE id = ${visitId}`);
            ...
            const checkoutMs = blockEndsAtDate.getTime();
            ...
            const activeVisit = await tx.execute((0, drizzle_orm_1.sql) `SELECT id FROM visits WHERE customer_id = ${customerId} AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1`);
            if (activeVisit.rows.length > 0) {
                const activeVisitId = activeVisit.rows[0].id;
                await resolveVisitBlocks(activeVisitId);
                const activeBlock = await tx.execute((0, drizzle_orm_1.sql) `SELECT cb.starts_at, cb.ends_at, cb.rental_type, r.number as resource_number, r.kind as resource_kind
           FROM checkin_blocks cb LEFT JOIN inventory_resources r ON cb.resource_id = r.id
           WHERE cb.visit_id = ${activeVisitId} ORDER BY cb.ends_at DESC LIMIT 1`);
                ...
                const waitlistResult = await tx.execute((0, drizzle_orm_1.sql) `SELECT id, desired_tier, backup_tier, status FROM waitlist WHERE visit_id = ${activeVisitId} AND status IN ('ACTIVE', 'OFFERED') ORDER BY created_at DESC LIMIT 1`);
                ...
                err.activeCheckin = {
                    ...
                    checkinAt: block?.starts_at ? ((0, utils_1.toDate)(block.starts_at)?.toISOString() ?? String(block.starts_at)) : null,
                    checkoutAt: block?.ends_at ? ((0, utils_1.toDate)(block.ends_at)?.toISOString() ?? String(block.ends_at)) : null,
                    overdue: block?.ends_at ? ((0, utils_1.toDate)(block.ends_at)?.getTime() ?? 0) < Date.now() : null,
                    currentTotalHours,
        ...
        const existingSession = await tx.execute((0, drizzle_orm_1.sql) `SELECT id, status FROM lane_sessions WHERE lane_id = ${laneId} AND status IN ('IDLE', 'ACTIVE', 'AWAITING_CUSTOMER') ORDER BY created_at DESC LIMIT 1`);
        let session;
        ...
        if (existingSession.rows.length > 0 && existingSession.rows[0].status !== 'COMPLETED') {
            ...
            const updateResult = await tx.execute((0, drizzle_orm_1.sql) `UPDATE lane_sessions SET customer_display_name = ${customerName}, membership_number = ${membershipNumber}, customer_id = ${customerId}, status = 'ACTIVE',
         ...
         WHERE id = ${existingId} RETURNING ${drizzle_orm_1.sql.raw(types_1.LANE_SESSION_COLS)}`);
            session = updateResult.rows[0];
        }
        else {
            const newSessionResult = await tx.execute((0, drizzle_orm_1.sql) `INSERT INTO lane_sessions (lane_id, status, staff_id, customer_id, customer_display_name, membership_number,
         ...
         ${desiredRentalTypeForSession}, NULL, NULL, NULL, ${selectionConfirmedForSession}, ${selectionConfirmedByForSession}, ${selectionLockedAtForSession}, ${flowStepForSession}, 0) RETURNING ${drizzle_orm_1.sql.raw(types_1.LANE_SESSION_COLS)}`);
        ...
        if (session.customer_id) {
            const customerInfo = await tx.execute((0, drizzle_orm_1.sql) `SELECT past_due_balance, membership_card_type, membership_valid_until FROM customers WHERE id = ${session.customer_id}`);
...
async function getLaneSessionSnapshot(laneId) {
    const row = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM lane_sessions WHERE lane_id = ${laneId} AND status IN ('ACTIVE', 'AWAITING_CUSTOMER', 'AWAITING_ASSIGNMENT', 'AWAITING_PAYMENT', 'AWAITING_SIGNATURE') ORDER BY created_at DESC LIMIT 1`);
    let sessionId = row.rows[0]?.id;
    if (!sessionId) {
        const completedRow = await db_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM lane_sessions WHERE lane_id = ${laneId} AND status = 'COMPLETED' AND (customer_id IS NOT NULL OR customer_display_name IS NOT NULL) ORDER BY updated_at DESC LIMIT 1`);
        sessionId = completedRow.rows[0]?.id;
    }
    if (!sessionId)
        return { session: null };
    const { payload } = await (0, payload_1.buildFullSessionUpdatedPayload)(sessionId);
